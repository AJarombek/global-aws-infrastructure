...
class TestJenkinsKubernetes(unittest.TestCase):
    ...
    def test_ecr_repository_exists(self) -> None:
        ...
        repos = self.ecr.describe_repositories(
            repositoryNames=["jenkins-jarombek-io"]
        ).get("repositories")
    ...
    def test_jenkins_jarombek_io_a_record_exists(self) -> None:
        ...
        try:
            a_record = Route53.get_record(
                zone_name="jarombek.io.",
                record_name="jenkins.jarombek.io.",
                record_type="A",
            )
        except IndexError:
            self.assertTrue(False)
        ...
        self.assertTrue(
            a_record.get("Name") == "jenkins.jarombek.io."
    ...
    def test_jenkins_load_balancer_security_group(self) -> None:
        sg = SecurityGroup.get_security_groups(name=f"jenkins-{ENV}-lb-security-group")[
            0
        ]
    ...
    def validate_jenkins_load_balancer_sg_rules(ingress: list, egress: list):
        ...
        return all(
            [len(ingress) == 2, ingress_80, ingress_443, len(egress) == 1, egress_neg1]
        )
