...
class TestLambdaLayers(unittest.TestCase):
    ...
    def test_upload_picture_layer_exists(self) -> None:
        ...
        matching_layers = [
            layer
            for layer in layers
            if layer.get("LayerName") == "upload-picture-layer"
        ]

        self.assertEqual(1, len(matching_layers))

        runtimes: List[str] = (
            matching_layers[0].get("LatestMatchingVersion").get("CompatibleRuntimes")
        )
    ...
    def test_aws_lambda_emails_layer_exists(self) -> None:
        ...
        matching_layers = [
            layer
            for layer in layers
            if layer.get("LayerName") == "aws-lambda-emails-layer"
        ]
