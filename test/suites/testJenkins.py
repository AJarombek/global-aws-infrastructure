...
class TestJenkins(unittest.TestCase):
    ...
    @unittest.skip(
        "Jenkins was moved from EC2 to EKS.  These resources may no longer exist."
    )
    def test_ami_exists(self) -> None:
        ...
        amis = self.ec2.describe_images(
            Owners=[owner],
            Filters=[{"Name": "name", "Values": ["global-jenkins-server*"]}],
        )
    ...
    def test_jenkins_launch_config_sg_valid(self) -> None:
        ...
        lcs = self.autoscaling.describe_launch_configurations(
            LaunchConfigurationNames=["global-jenkins-server-lc"], MaxRecords=1
        )
        ...
        sg = self.ec2.describe_security_groups(GroupIds=[sg_id]).get("SecurityGroups")[
            0
        ]
    ...
    def test_jenkins_load_balancer_valid(self) -> None:
        ...
        """
        self.assertTrue(
            EC2.load_balancer_target_groups_valid(
                asg_name="global-jenkins-server-asg",
    ...
    def test_jenkins_load_balancer_sg_valid(self) -> None:
        ...
        try:
            sg = SecurityGroup.get_security_groups(
                "global-jenkins-server-lb-security-group"
            )[0]
        except IndexError:
            self.assertTrue(False)
            return
